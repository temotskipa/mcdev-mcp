...
export async function fetchVersionManifest(): Promise<MojangVersionManifest> {
  ...
  manifestPromise = (async () => {
    try {
      ...
      throw new Error(`Failed to fetch Mojang version manifest (${MOJANG_VERSION_MANIFEST_URL}): ${describeAxiosError(e)}`, { cause: e });
    }
  })();
...
export async function fetchVersionDetail(versionId: string): Promise<MojangVersionDetail> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch Mojang version detail for ${versionId} (${version.url}): ${describeAxiosError(e)}`, { cause: e });
  }
}
...
export async function fetchFabricApiVersions(): Promise<string[]> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch Fabric API maven metadata (${url}): ${describeAxiosError(e)}`, { cause: e });
  }

  // The Fabric Maven metadata XML is generated by Maven's standard tooling
  // and uses a fixed `<version>X</version>` shape — no whitespace, no CDATA,
  // no nested elements. A regex is sufficient and avoids pulling in an XML
  // parser dep for one call site. We still:
  //   * `.trim()` each match to be tolerant of whitespace-only nodes that a
  //     future Maven tweak might emit
  //   * filter out any capture that LOOKS like a CDATA wrapper, so if the
  //     format ever does grow `<![CDATA[...]]>` we surface the bug instead
  //     of indexing literal "<![CDATA[..." strings as version numbers
