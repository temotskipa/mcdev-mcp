...
public final class SourcePublicationProcessMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var lease = VersionOperationLease.write(paths, version)) {
            if (stoppingPoint.equals("RECOVER")) publisher.recover(paths, version, lease);
            else {
                SourceIndexSnapshot before = publisher.captureBefore(paths, version, lease, Cancellation.none());
                publisher.publish(paths, version, lease, transaction, transaction.resolve("candidate/client"),
                        transaction.resolve("candidate/symbols.mv.db"), transaction.resolve("candidate/source-preparation.json"), before, Cancellation.none());
            }
        }
    }
}
