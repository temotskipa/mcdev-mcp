...
public final class DirectoryAliasFixture {
    ...
    public static void create(Path alias, Path target) throws Exception {
        if (System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("win")) {
            ...
            try {
                ...
                if (process.exitValue() != 0) throw new IOException("Junction creation failed: " + new String(process.getInputStream().readAllBytes(), java.nio.charset.StandardCharsets.UTF_8));
            } finally {
                if (process.isAlive()) {
    ...
    public static void remove(Path alias) throws IOException {
        ...
        if (!attributes.isSymbolicLink() && !attributes.isOther()) throw new IOException("Refusing to remove a real directory as an alias: " + alias);
        Files.delete(alias);
    }
}
