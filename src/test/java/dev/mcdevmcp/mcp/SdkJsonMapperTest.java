...
class SdkJsonMapperTest {
    ...
    void mapsTopLevelRecordsAndStructuredJdkValues() throws Exception {
        var expected = new SdkJsonProbe(
                URI.create("https://example.test/api"),
                SdkJsonMode.FAST,
                List.of(new SdkJsonItem("first", 1), new SdkJsonItem("second", 2)),
                new BigInteger("1234567890123456789012345678901234567890"),
                new BigDecimal("1234567890.012345678901234567890"),
    ...
    void ignoresUnknownFieldsForStructuredRecords() throws Exception {
        assertEquals(
                new SdkJsonUnknownFieldProbe("known"),
                MAPPER.readValue("{\"name\":\"known\",\"unknown\":true}", SdkJsonUnknownFieldProbe.class));
    }
}
...
record SdkJsonItem(String name, int position) {
}
