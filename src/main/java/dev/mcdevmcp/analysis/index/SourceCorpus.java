    ...
    static SourceCorpus discover(List<SourceRoot> roots, Cancellation cancellation) throws IOException, InterruptedException {
        ...
        for (SourceRoot root : roots) {
            ...
            try (var paths = Files.walk(root.path())) {
                List<Path> javaSources = paths.filter(path -> Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)).filter(path -> path.getFileName().toString().endsWith(".java")).sorted(Comparator.comparing(path -> portable(root.path().relativize(path)))).toList();
                for (Path source : javaSources) {
                    cancellation.throwIfCancelled();
                    Path relative = root.path().relativize(source).normalize();
                    String relativeName = portable(relative);
