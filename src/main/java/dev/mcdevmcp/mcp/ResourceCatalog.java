...
public final class ResourceCatalog {
    ...
    private final List<ResourceDefinition> definitions = List.of(new ResourceDefinition(URI.create("mcdev://guides/python-scripting"), "python-scripting", "Python client guide for the Minecraft DebugBridge", "How to drive a live Minecraft instance from Python by speaking the DebugBridge WebSocket protocol directly — wire format, a minimal asyncio client, and the Groovy surface you send through it. Read this before writing a Python script that bypasses the mcdev-mcp tools.", "text/markdown", "/guides/python-scripting.md"), new ResourceDefinition(URI.create("mcdev://guides/dev-loop"), "dev-loop", "The mod dev loop: build → deploy → relaunch → rejoin", "How a coding agent runs the full mod test loop without human interaction: discover the launcher and instance from the bridge's gameDir, deploy the jar, quit via mc_quit_client, launch detached from the shell, reconnect with mc_wait_for_bridge, and rejoin a server. Read this before restarting the Minecraft client or automating mod testing.", "text/markdown", "/guides/dev-loop.md"));
    ...
    public ResourceRead read(URI uri) {
        ...
        return new ResourceRead(definition.uri(), definition.mimeType(), nodeLineEndings(JsonSupport.readTextResource(definition.classpathResource())));
    }
}
