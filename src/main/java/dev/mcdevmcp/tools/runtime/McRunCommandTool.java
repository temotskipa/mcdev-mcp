...
final class McRunCommandTool {
    ...
    static ToolBinding<RunCommandArguments> binding(RuntimeToolSupport runtime, SessionControlSupport sessionControl) {
        ...
        return ToolBinding.compatibility(decoder, (arguments, _) -> SessionControlSupport.recoverTool(SessionControlSupport.mapCancellable(sessionControl.send(ENDPOINT, RuntimeToolSupport.payload("command", stripSlash(arguments.command())), null), response -> {
