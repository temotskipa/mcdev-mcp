...
public final class ElementKindCodec {
    ...
    public static ElementKind fromWireName(String wireName) {
        ...
        return WIRE_NAMES.entrySet().stream()
                .filter(entry -> entry.getValue().equals(wireName))
                .map(Map.Entry::getKey)
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Unsupported type kind: " + wireName));
    }
