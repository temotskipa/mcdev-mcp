...
public final class CachePathBoundary {
    ...
    public Path logicalRoot() { return logicalRoot; }

    public Path physicalRoot() { return physicalRoot; }

    public PlatformPaths resolvedPaths() { return new PlatformPaths(physicalRoot); }

    public void revalidate() throws IOException {
        BasicFileAttributes attributes = directory(physicalRoot);
        if (!physicalRoot.toRealPath().equals(physicalRoot) || !Objects.equals(fileKey, attributes.fileKey())
    ...
    public void require(PlatformPaths paths) throws IOException {
        ...
        if (!selected.equals(logicalRoot) && !selected.equals(physicalRoot)) throw new IOException("Cache paths do not belong to the held boundary: " + selected);
        revalidate();
    }

    public Path require(Path child) throws IOException { return resolve(child); }
    ...
    public Path resolve(Path child) throws IOException {
        ...
        Path relative;
        if (normalized.startsWith(physicalRoot)) relative = physicalRoot.relativize(normalized);
        else if (normalized.startsWith(logicalRoot)) relative = logicalRoot.relativize(normalized);
        else throw new IOException("Path is outside the selected cache boundary: " + child);
        ...
        for (Path component : relative) {
            ...
            try { attributes = Files.readAttributes(current, BasicFileAttributes.class, LinkOption.NOFOLLOW_LINKS); }
            catch (NoSuchFileException absent) { continue; }
            if (attributes.isSymbolicLink() || attributes.isOther()
                    || !attributes.isDirectory() && (!attributes.isRegularFile() || index < relative.getNameCount())
    ...
    private static BasicFileAttributes directory(Path path) throws IOException {
        ...
        if (!attributes.isDirectory() || attributes.isSymbolicLink() || attributes.isOther()) throw new IOException("Cache boundary must be a real directory: " + path);
        return attributes;
    }
}
