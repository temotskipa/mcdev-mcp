...
public final class SourceIndexTransactionPublisher {
    ...
    public SourceIndexTransactionPublisher() { this((event, path) -> { }); }

    SourceIndexTransactionPublisher(SourcePublicationHook hook) { this(hook, null); }
    ...
    public SourceIndexSnapshot captureBefore(PlatformPaths paths, MinecraftVersion version,
                                             VersionOperationLease lease, Cancellation cancellation) throws IOException {
        requireWrite(paths, version, lease);
        if (boundary == null) return new SourceIndexTransactionPublisher(hook, lease.boundary()).captureBefore(lease.resolvedPaths(), version, lease, cancellation);
    ...
    public SourcePublicationResult publish(PlatformPaths paths, MinecraftVersion version, VersionOperationLease lease,
                                           Path transactionRoot, Path candidateSource, Path candidateDatabase,
                                           Path candidateStamp, SourceIndexSnapshot expectedBefore,
                                           Cancellation cancellation) throws IOException {
        return publish(paths, version, lease, transactionRoot, candidateSource, candidateDatabase, candidateStamp,
                expectedBefore, SourcePublicationMode.REPLACE_SOURCES, cancellation);
    }

    public SourcePublicationResult publish(PlatformPaths paths, MinecraftVersion version, VersionOperationLease lease,
                                           Path transactionRoot, Path candidateSource, Path candidateDatabase,
                                           Path candidateStamp, SourceIndexSnapshot expectedBefore,
                                           SourcePublicationMode mode, Cancellation cancellation) throws IOException {
        requireWrite(paths, version, lease);
        if (boundary == null) return new SourceIndexTransactionPublisher(hook, lease.boundary()).publish(lease.resolvedPaths(), version, lease,
                transactionRoot, candidateSource, candidateDatabase, candidateStamp, expectedBefore, mode, cancellation);
        boundary.require(paths);
        ...
        if (mode == SourcePublicationMode.REPLACE_SOURCES) requireCandidate(transaction, candidateSource);
        ...
        if (candidateSource.startsWith(candidateDatabase) || candidateDatabase.startsWith(candidateSource)
                || candidateStamp.startsWith(candidateSource) || candidateSource.startsWith(candidateStamp)
                || candidateStamp.equals(candidateDatabase)) throw new IOException("Overlapping candidate paths");
        try (var lock = canonicalDatabaseLock(paths, version)) {
            if (!lock.isHeld()) throw new IOException("Database publication lock not held");
            refusePending(paths, version);
            ...
            if (mode == SourcePublicationMode.REUSE_SOURCES) verify(expectedBefore.source(), source, "Reused source changed");
            ...
            var journal = new SourcePublicationJournal(1, version, paths.cacheRoot().toAbsolutePath().normalize().toString(),
                    UUID.fromString(transaction.getFileName().toString()), SourcePublicationPhase.PREPARED, mode, expectedBefore, candidate);
            ...
            try {
                ...
                if (mode == SourcePublicationMode.REPLACE_SOURCES) detach(paths.sourceRoot(version), held.resolve("client"), expectedBefore.source());
        ...
        try (var lock = canonicalDatabaseLock(paths, version)) {
            ...
            try {
                if (journal.phase() == SourcePublicationPhase.COMMITTED) {
                    ...
                    archiveCommitted(transaction, journal);
                    clearPending(paths, version, journal);
                } else rollback(paths, version, transaction, journal);
    ...
    private void rollback(PlatformPaths paths, MinecraftVersion version, Path transaction, SourcePublicationJournal journal) throws IOException {
        ...
        } else verify(journal.before().source(), inventory(paths.sourceRoot(version), Cancellation.none()), "Reused source changed; recovery remains pending");
        Set<String> names = new TreeSet<>(managedNames(journal.before().index()));
        names.add(DATABASE);
        for (String name : names) {
            restore(paths.indexRoot(version).resolve(name), transaction.resolve("old/index").resolve(name), failed.resolve(name),
                    entryInventory(journal.before().index(), name), entryInventory(journal.candidate().index(), name));
        }
    ...
    private void restore(Path canonical, Path snapshot, Path failed, SourceTreeInventory before, SourceTreeInventory candidate) throws IOException {
        ...
        if (actual.present()) {
            if (!actual.equals(candidate)) throw new IOException("Unknown canonical contents preserved at " + canonical);
    ...
    private SourcePublicationJournal advance(PlatformPaths paths, MinecraftVersion version, Path transaction,
                                              SourcePublicationJournal journal, SourcePublicationPhase phase, Cancellation cancellation) throws IOException {
        SourceTreeInventory.checkCancelled(cancellation);
    ...
    private SourceIndexSnapshot capture(PlatformPaths paths, MinecraftVersion version, Cancellation cancellation) throws IOException {
        ...
        if (Files.exists(checked(active), LinkOption.NOFOLLOW_LINKS)) throw new IOException("Active H2 companion prevents migration: " + active);
        ...
        return new SourceIndexSnapshot(source,
                SourceTreeInventory.capture(paths.indexRoot(version), boundary, cancellation, Set.of(DATABASE_LOCK)),
                stamp);
    }
    ...
    private static List<String> managedNames(SourceTreeInventory index) throws IOException {
        ...
        for (SourceInventoryEntry entry : index.entries()) if (managed(entry.relativePath())) {
            if (entry.kind() != SourceEntryKind.FILE || entry.relativePath().contains("/")) throw new IOException("Unsafe H2 artifact: " + entry.relativePath());
    ...
    private static boolean managed(String name) {
        if (Set.of(DATABASE, DATABASE + ".bak", "symbols.newFile", "symbols.tempFile", "symbols.lock.db",
                "symbols.trace.db", "symbols.trace.db.old").contains(name)) return true;
        if (!name.startsWith("symbols.") || !name.endsWith(".temp.db") || name.length() <= "symbols.".length() + ".temp.db".length()) return false;
    ...
    private static SourceTreeInventory entryInventory(SourceTreeInventory index, String name) {
        return index.entries().stream().filter(entry -> entry.relativePath().equals(name)).findFirst()
                .map(entry -> SourceTreeInventory.of(true, List.of(new SourceInventoryEntry("", entry.kind(), entry.size(), entry.sha256()))))
                .orElseGet(() -> SourceTreeInventory.of(false, List.of()));
    }
    ...
    private void validateCommitted(PlatformPaths paths, MinecraftVersion version, Path transaction, SourcePublicationJournal journal) throws IOException {
        ...
        verifyRetained(transaction, journal.before());
        if (journal.mode() == SourcePublicationMode.REPLACE_SOURCES) verify(journal.before().source(), inventory(transaction.resolve("held/client"), Cancellation.none()), "Held original source changed");
        for (String name : managedNames(journal.before().index())) verify(entryInventory(journal.before().index(), name), inventory(transaction.resolve("held/index").resolve(name), Cancellation.none()), "Held original H2 artifact changed");
    ...
    private void validatePair(PlatformPaths paths, MinecraftVersion version, Path source, Path database, Path stamp, SourceTreeInventory inventory) throws IOException {
        ...
        if (!filename.endsWith(".mv.db") || database.toString().contains(";")) throw new IOException("Invalid candidate database path");
        ...
        try (var siblings = Files.list(checked(database.getParent()))) {
            for (Path sibling : siblings.toList()) {
                ...
                if (name.startsWith(baseName + ".") && managed("symbols" + name.substring(baseName.length())) && !name.equals(filename)) throw new IOException("Candidate H2 companion remains: " + sibling);
            }
        }
        ...
        if (!Path.of(preparation.inputs().remappedJar().path()).toAbsolutePath().normalize().equals(paths.remappedJar(version).toAbsolutePath().normalize())) throw new IOException("Candidate stamp remapped JAR path differs");
        ...
        try (var connection = DriverManager.getConnection("jdbc:h2:file:" + base + ";ACCESS_MODE_DATA=r;IFEXISTS=TRUE;DB_CLOSE_ON_EXIT=FALSE;FILE_LOCK=FS;TRACE_LEVEL_FILE=0")) {
            ...
            try (var statement = connection.createStatement(); var row = statement.executeQuery("SELECT minecraft_version,source_root,remapped_jar_sha256 FROM metadata")) {
                if (!row.next() || !version.value().equals(row.getString(1)) || !paths.sourceRoot(version).toAbsolutePath().normalize().equals(Path.of(row.getString(2)).toAbsolutePath().normalize())
                        || !preparation.inputs().remappedJar().sha256().equals(row.getString(3)) || row.next()) throw new IOException("Candidate database final source identity differs");
            }
            try (var statement = connection.createStatement(); var rows = statement.executeQuery("SELECT DISTINCT source_path FROM types WHERE source_namespace = 'minecraft'")) {
                while (rows.next()) {
                    Path relative = Path.of(rows.getString(1));
                    Path resolved = source.toAbsolutePath().normalize().resolve(relative).normalize();
                    if (relative.isAbsolute() || !resolved.startsWith(source.toAbsolutePath().normalize()) || !Files.isRegularFile(checked(resolved), LinkOption.NOFOLLOW_LINKS)) throw new IOException("Candidate index source path is unsafe or missing: " + relative);
                }
            }
        } catch (SQLException exception) { throw new IOException("Invalid candidate symbol database", exception); }
    ...
    private void validateInput(SourceArtifactIdentity artifact) throws IOException {
        ...
        if (actual.size() != artifact.size() || !actual.sha256().equals(artifact.sha256())) throw new IOException("Source input changed: " + artifact.path());
    }
    ...
    private void copy(Path source, Path target, SourceTreeInventory inventory, Cancellation cancellation) throws IOException {
        ...
        for (SourceInventoryEntry entry : inventory.entries()) {
            ...
            checked(from);
            checked(to);
            if (entry.kind() == SourceEntryKind.DIRECTORY) Files.createDirectory(to);
            else {
                Files.createDirectories(checked(to.getParent()));
                checked(to);
                Files.copy(from, to, LinkOption.NOFOLLOW_LINKS);
                try (var channel = FileChannel.open(to, StandardOpenOption.WRITE)) { channel.force(true); }
            }
        }
    ...
    private void move(Path source, Path target) throws IOException {
        ...
        checked(target);
        if (!Files.getFileStore(source).equals(Files.getFileStore(target.getParent()))) throw new IOException("Cross-filesystem migration move refused");
        if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) throw new FileAlreadyExistsException(target.toString());
    ...
    private void preflightStore(PlatformPaths paths, MinecraftVersion version, Path transaction, Path... candidates) throws IOException {
        ...
        for (Path path : candidates) if (!store.equals(Files.getFileStore(path))) throw new IOException("Cross-filesystem migration candidate refused");
        Files.createDirectories(checked(paths.versionCache(version)));
        if (!store.equals(Files.getFileStore(paths.versionCache(version))) || !store.equals(Files.getFileStore(paths.indexRoot(version)))) throw new IOException("Cross-filesystem migration target refused");
        ...
        try {
            ...
            if (!Arrays.equals(Files.readAllBytes(source), JSON.writeValueAsBytes("source"))
                    || !Arrays.equals(Files.readAllBytes(occupied), JSON.writeValueAsBytes("occupied"))) throw new IOException("Provider changed preflight artifacts on a refused move");
        }
    ...
    private Path validateTransaction(PlatformPaths paths, MinecraftVersion version, Path transaction) throws IOException {
        ...
        if (!Objects.equals(normalized.getParent(), migrations(paths, version).toAbsolutePath().normalize())) throw new IOException("Invalid migration transaction location");
        try { if (!UUID.fromString(normalized.getFileName().toString()).toString().equals(normalized.getFileName().toString())) throw new IllegalArgumentException(); }
        catch (IllegalArgumentException exception) { throw new IOException("Invalid migration transaction identifier", exception); }
        checked(normalized);
        return normalized;
    }
    ...
    private void requireCandidate(Path transaction, Path candidate) throws IOException {
        ...
        if (!normalized.startsWith(transaction) || normalized.equals(transaction)) throw new IOException("Candidate outside migration transaction: " + candidate);
    ...
    private static void requireDirectory(SourceTreeInventory inventory, String label) throws IOException {
        if (!inventory.present() || inventory.entries().getFirst().kind() != SourceEntryKind.DIRECTORY) throw new IOException("Expected " + label + " directory");
    }

    private static void requireFile(SourceTreeInventory inventory, String label) throws IOException {
        if (!inventory.present() || inventory.entries().size() != 1 || inventory.entries().getFirst().kind() != SourceEntryKind.FILE) throw new IOException("Expected " + label + " regular file");
    }

    private static void verify(Object expected, Object actual, String label) throws IOException { if (!expected.equals(actual)) throw new IOException(label); }

    private void refusePending(PlatformPaths paths, MinecraftVersion version) throws IOException {
        if (Files.exists(checked(pendingPath(paths, version)), LinkOption.NOFOLLOW_LINKS)) throw new IOException("Source/index recovery required at " + pendingPath(paths, version));
    }
    ...
    private SourcePublicationJournal readJournal(PlatformPaths paths, MinecraftVersion version) throws IOException {
        ...
        try {
            ...
            if (!Arrays.equals(bytes, JSON.writeValueAsBytes(journal))) throw new IOException("Noncanonical migration journal");
            if (!journal.minecraftVersion().equals(version) || !journal.cacheRoot().equals(paths.cacheRoot().toAbsolutePath().normalize().toString())) throw new IOException("Migration journal identity mismatch");
            ...
            if (journal.mode() == SourcePublicationMode.REUSE_SOURCES) verify(journal.before().source(), journal.candidate().source(), "Journal changes reused sources");
            Path transaction = validateTransaction(paths, version, migrations(paths, version).resolve(journal.transactionId().toString()));
            ...
            if (!Arrays.equals(transactionBytes, JSON.writeValueAsBytes(transactionJournal))) throw new IOException("Noncanonical transaction journal");
            if (!journal.withPhase(transactionJournal.phase()).equals(transactionJournal)
                    || transactionJournal.phase().ordinal() < journal.phase().ordinal()
                    || transactionJournal.phase().ordinal() > journal.phase().ordinal() + 1) throw new IOException("Conflicting migration journal phases");
            ...
            return journal;
        } catch (RuntimeException exception) { throw new IOException("Corrupt migration journal at " + pointer, exception); }
    }
    ...
    private static Path migrations(PlatformPaths paths, MinecraftVersion version) { return paths.cacheRoot().resolve("migrations").resolve(version.value()); }
    private static Path pendingPath(PlatformPaths paths, MinecraftVersion version) { return migrations(paths, version).resolve("pending.json"); }
    private static Path stampPath(PlatformPaths paths, MinecraftVersion version) { return paths.versionCache(version).resolve("source-preparation.json"); }
}
